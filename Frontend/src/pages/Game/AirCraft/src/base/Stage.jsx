...
class Stage extends Component {
  ...
  componentDidMount() {
    let elm = document.getElementById("stage")
    this.setState({
      gameProperties: { eidth: elm.offsetWidth, height: elm.offsetHeight }
    })
  ...
  removeSprite = sprite => {
    ...
    const newList = this.state.children.filter(
      item => item.props.pKey !== spriteKey
    )
  ...
  handleColision(currentSprite) {
    this.sprites.map(sprite => {
      if (sprite !== currentSprite) {
        if (
          !sprite ||
          (sprite.props.startAt && sprite.props.startAt > this.frameCount)
        ) { return true }
  ...
  getRelativeCoordinates(position, element) {
    const offset = {
