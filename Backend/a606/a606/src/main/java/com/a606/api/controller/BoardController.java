...
public class BoardController {
    ...
    public List<?> searchList(@RequestParam(required = true) @ApiParam(example = "all") String tab, @RequestParam(required = true) @ApiParam(example = "id") String order, @RequestParam(required = true) @ApiParam(example = "false") boolean isDesc, @RequestParam(required = true) @ApiParam(example = "1") int pageNo, @RequestParam(required = true) @ApiParam(example = "10") int pageSize) throws Exception {
        return boardService.getNFTList(tab, order, isDesc, pageNo, pageSize);
    }
